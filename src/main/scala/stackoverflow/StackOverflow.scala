...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    ...
    // val answers = postings.filter(_.parentId.isDefined).groupBy(_.parentId.get)
    val answers: RDD[(QID, Answer)] = postings.filter(a => a.parentId.isDefined && a.postingType == 2).map(a => (a.parentId.get, a))


    // val partitioner = new RangePartitioner(questions.partitions.size, questions)
    // questions.partitionBy(partitioner)
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] = {
    ...
    // toList to transform the Iterable to an immutable Iterable so matching is possible
    grouped.map(_._2.toList).flatMap {
      case (q, a) :: tail => Some((q, answerHighScore((a :: tail.map(_._2)).toArray)))
      case Nil => None
    }
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] = {
    ...
    // ha-ha moment here due to use of groupByKey but carefull the map before destroys any partitionning

    // !!! size of newMeans is less or equal to oldMeans in size , if none of the vectors are associated to some old mean
    // we must pad with ??? old mean ? or infinite distance (max distance)
    val newComputedMeans = vectors.map(v => (findClosest(v, means), v)).groupByKey.mapValues(averageVectors).collect()
  ...
  def clusterResults(means: Array[(LangIndex, HighScore)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      // println("langLabel : " + langLabel)
      // println("langPercent : " + langPercent)
      // println("medianScore : " + medianScore)
